...
config_path = filepath + '\\chinese_L-12_H-768_A-12\\bert_config.json'
checkpoint_path = filepath + '\\chinese_L-12_H-768_A-12\\bert_model.ckpt'
dict_path = filepath + '\\chinese_L-12_H-768_A-12\\vocab.txt'
...
def seq_padding(X, padding=0):
    ...
    L = [len(x) for x in X]
    ML = max(L)
    # 获取所有文本tokenize之后的最大词量，对其他不够长的句子用零补充
    # concatenate方法用于向量的拼接，参数axis默认为0，即在行上进行扩展
    return np.array([
...
class data_generator:
    ...
    def __iter__(self):
        while True:
            ...
            # X1 X2 是对每一条数据的文本 分词 向量化 后得到的矩阵
            X1, X2, S1, S2 = [], [], [], []
            for i in idxs:
                d = self.data[i]
                # todo d[0]为什么是text 不是文本id吗
